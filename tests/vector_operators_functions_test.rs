...
fn test_vector_basic_operators() {
    ...
    println!("测试1: 向量加法 (+)");
    let add_result =
        db.sql_query("SELECT vector3 + vector3 FROM VECTOR_OPS_FUNC_TABLE WHERE id = 1");
    if add_result.is_ok() {
        println!("  向量加法 (+) 语法验证成功");
    } else {
        println!("  向量加法 (+) 语法验证失败");
    ...
    let field_mul_result =
        db.sql_query("SELECT vector3 * scalar FROM VECTOR_OPS_FUNC_TABLE WHERE id = 1");
...
fn test_vector_comparison_operators() {
    ...
    println!("测试1: 向量相等 (=)");
    let eq_result =
        db.sql_query("SELECT id FROM VECTOR_OPS_FUNC_TABLE WHERE vector3 = [1.0, 2.0, 3.0]");
    if eq_result.is_ok() {
        println!("  向量相等 (=) 语法验证成功");
    } else {
        println!("  向量相等 (=) 语法验证失败");
    ...
    println!("测试3: 向量小于 (<)");
    let lt_result =
        db.sql_query("SELECT id FROM VECTOR_OPS_FUNC_TABLE WHERE vector3 < [2.0, 4.0, 6.0]");
    if lt_result.is_ok() {
        println!("  向量小于 (<) 语法验证成功");
    } else {
        println!("  向量小于 (<) 语法验证失败");
...
fn test_vector_search_functions() {
    ...
    let similar_dist_result = db.sql_query(
        "SELECT id FROM VECTOR_OPS_FUNC_TABLE WHERE VECTOR_SIMILAR(vector3, [2.0, 4.0, 6.0], L2)",
    );
