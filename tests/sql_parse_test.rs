...
#[test]
fn test_create_checkpoint_parse() {
    let sql = "CREATE CHECKPOINT;";
    ...
    match parse_sql_query(sql) {
        Ok(query) => {
            println!("✓ Successfully parsed query: {:?}", query);
            assert_eq!(query.query_type, remdb::sql::QueryType::CreateCheckpoint);
        }
        Err(err) => {
            println!("✗ Parse error: {:?}", err);
            // 获取更详细的错误信息
            ...
            panic!("Failed to parse CREATE CHECKPOINT statement");
        }
    }
}
