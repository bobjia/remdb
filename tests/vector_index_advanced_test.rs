...
fn test_vector_index_simple_update() {
    ...
    let insert_id = table.insert(&initial_record as *const _ as *const u8).unwrap();
    assert!(insert_id < config.tables[0].max_records);
    println!("成功插入 1 条初始向量数据");
    
    // 创建向量索引
    println!("创建初始向量索引");
    let result = db.sql_query("CREATE INDEX vector_simple_update_idx ON VECTOR_ADVANCED_TABLE (vector) USING HNSW");
...
fn test_vector_index_simple_config() {
    ...
    let query = "CREATE INDEX vector_simple_config_idx ON VECTOR_ADVANCED_TABLE (vector) USING HNSW";
...
fn test_vector_index_simple_boundary() {
    ...
    let table = db.get_table_mut(0).unwrap();
    let insert_id = table.insert(&single_record as *const _ as *const u8).unwrap();
    assert!(insert_id < config.tables[0].max_records);
    println!("成功插入单个向量数据");
    
    // 创建索引
    let result = db.sql_query("CREATE INDEX vector_simple_boundary_idx ON VECTOR_ADVANCED_TABLE (vector) USING HNSW");
...
fn test_vector_index_simple_dimension() {
    ...
    // 测试1: 创建向量索引
    println!("测试1: 创建向量索引");
    let result = db.sql_query("CREATE INDEX vector_simple_dim_idx ON VECTOR_ADVANCED_TABLE (vector) USING HNSW");
