...
fn test_vector_hybrid_search_basic() {
    ...
    // 创建向量索引
    let result = db.sql_query("CREATE INDEX vector_hybrid_idx ON VECTOR_HYBRID_TABLE (vector) USING HNSW");
    if result.is_ok() {
        println!("成功创建向量索引");
    } else {
...
fn test_vector_hybrid_search_multiple_conditions() {
    ...
    for i in 1..=10 {
        let mut record = VectorHybridRecord {
            ...
            category: if i % 3 == 0 { 3 } else if i % 2 == 0 { 2 } else { 1 },
    ...
    // 创建向量索引
    let result = db.sql_query("CREATE INDEX vector_hybrid_multi_idx ON VECTOR_HYBRID_TABLE (vector) USING HNSW");
    if result.is_ok() {
        println!("成功创建向量索引");
    } else {
        println!("创建向量索引失败，可能功能尚未实现");
    ...
    let multi_cond_result = db.sql_query("SELECT id FROM VECTOR_HYBRID_TABLE WHERE category = 1 AND score > 0.8");
    ...
    let bool_cond_result = db.sql_query("SELECT id FROM VECTOR_HYBRID_TABLE WHERE active = true AND category = 2");
...
fn test_vector_hybrid_search_different_data_types() {
    ...
    // 创建向量索引
    let idx_result = db.sql_query("CREATE INDEX vector_hybrid_types_idx ON VECTOR_HYBRID_TABLE (vector) USING HNSW");
    if idx_result.is_ok() {
        println!("成功创建向量索引");
    }
    ...
    // 测试1: 浮点数范围条件
    println!("测试1: 浮点数范围条件");
    let float_range_result = db.sql_query("SELECT id FROM VECTOR_HYBRID_TABLE WHERE score BETWEEN 0.8 AND 0.95");
...
fn test_vector_hybrid_search_complex_conditions() {
    ...
    let and_result = db.sql_query("SELECT id FROM VECTOR_HYBRID_TABLE WHERE category = 1 AND score > 0.7 AND active = true");
    if and_result.is_ok() {
...
fn test_vector_index_with_secondary_index() {
    ...
    // 创建向量索引
    let vec_idx_result = db.sql_query("CREATE INDEX vector_with_secondary_idx ON VECTOR_HYBRID_TABLE (vector) USING HNSW");
    ...
    let composite_result = db.sql_query("SELECT id FROM VECTOR_HYBRID_TABLE WHERE category = 1 AND score > 0.75");
