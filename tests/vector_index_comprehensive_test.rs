...
fn test_vector_index_stats() {
    ...
    // 测试1: 创建向量索引
    println!("测试1: 创建向量索引");
    let result = db.sql_query("CREATE INDEX vector_stats_idx ON VECTOR_TABLE_2D (vector) USING HNSW");
    assert!(result.is_ok(), "创建向量索引应该成功");
    println!("成功创建向量索引");
...
fn test_vector_index_range_query() {
    ...
    // 测试1: 创建向量索引
    println!("测试1: 创建向量索引");
    let result = db.sql_query("CREATE INDEX vector_range_idx ON VECTOR_TABLE_2D (vector) USING HNSW");
    assert!(result.is_ok(), "创建向量索引应该成功");
    println!("成功创建向量索引");
...
fn test_vector_index_large_scale() {
    ...
    // 测试1: 创建向量索引
    println!("测试1: 创建向量索引");
    let result = db.sql_query("CREATE INDEX vector_large_idx ON VECTOR_TABLE_2D (vector) USING HNSW");
    assert!(result.is_ok(), "创建向量索引应该成功");
    println!("成功创建向量索引");
...
fn test_vector_index_error_handling() {
    ...
    let result = db.sql_query("CREATE INDEX non_vector_idx ON VECTOR_TABLE_2D (category) USING HNSW");
    ...
    // 先创建一个有效的向量索引
    let result = db.sql_query("CREATE INDEX vector_first_idx ON VECTOR_TABLE_2D (vector) USING HNSW");
    assert!(result.is_ok(), "创建第一个向量索引应该成功");
    println!("成功创建第一个向量索引");
    
    // 尝试创建第二个向量索引，应该失败
    let result = db.sql_query("CREATE INDEX vector_second_idx ON VECTOR_TABLE_2D (vector) USING HNSW");
    // 应该失败，因为每个表只支持一个索引
    ...
    let record = VectorRecord { id: 1, vector: [1.0, 2.0], category: 1 };
    let table = db.get_table_mut(0).unwrap();
