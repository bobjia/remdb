...
fn test_time_series_config() {
    ...
    assert_eq!(
        config.partition_duration().as_secs(),
        3600,
        "默认分区时长应为1小时"
    );
    assert_eq!(
        config.retention_period().as_secs(),
        7 * 24 * 3600,
        "默认保留期应为7天"
    );
    assert_eq!(
    ...
    assert_eq!(
        custom_config.partition_duration().as_secs(),
        1800,
        "自定义分区时长应为30分钟"
    );
    assert_eq!(
        custom_config.retention_period().as_secs(),
        7 * 24 * 3600,
        "自定义保留期应为7天"
    );
    assert_eq!(
        custom_config.compression,
...
fn test_compression_algorithms() {
    ...
    assert!(
        run_compressed.len() < run_values.len() * 8,
        "Run-Length编码应该能压缩重复数据"
    );

    let run_decompressed = decompress_run_length(&run_compressed);
    assert_eq!(
        run_decompressed, run_values,
        "Run-Length解码应能正确恢复原始数据"
    );
    ...
    assert!(
        float_compressed.len() == float_values.len() * 8,
        "浮点数Delta编码应该保持相同大小"
    );
    ...
    for i in 0..float_values.len() {
        assert!(
            (float_decompressed[i] - float_values[i]).abs() < 0.0001,
            "浮点数Delta解码应能正确恢复原始数据"
        );
    }
}
...
fn test_lifecycle_manager() {
    ...
    assert!(
        !manager.is_expired(recent_timestamp),
        "12小时前的数据不应过期"
    );
    ...
    assert!(
        manager.is_expired(expired_timestamp),
        "25小时前的数据应过期"
    );
    ...
    assert!(
        !manager.is_expired(boundary_timestamp),
        "刚好24小时前的数据不应过期"
    );
    ...
    assert!(
        manager.is_expired(over_timestamp),
        "超过24小时前的数据应过期"
    );
...
fn test_partition_manager() {
    ...
    // 检查分区是否不同
    assert!(
        !std::ptr::eq(partition1.as_ref(), partition2.as_ref()),
        "不同时间应创建不同分区"
    );
    assert!(
        !std::ptr::eq(partition2.as_ref(), partition3.as_ref()),
        "不同时间应创建不同分区"
    );
    ...
    assert!(
        std::ptr::eq(partition1.as_ref(), same_partition.as_ref()),
        "同一时间应返回相同分区"
    );
...
fn test_time_series_record() {
    ...
    assert_eq!(
        cloned_record.timestamp, record.timestamp,
        "克隆记录的时间戳应相同"
    );
...
fn test_time_series_integration() {
    unsafe {
        ...
        assert!(
            matches!(compression, CompressionType::Delta),
            "压缩类型应正确"
        );
    ...
    unsafe {
        ...
        if crate::platform::PLATFORM.get().is_none() {
            ...
            impl crate::platform::Platform for TestPlatform {
                ...
                ) -> crate::platform::FileResult<crate::platform::FileHandle> {
                    Err(())
                }

                fn file_close(
                    &self,
                    _handle: crate::platform::FileHandle,
                ) -> crate::platform::FileResult<()> {
                    Err(())
                }
                ...
                fn file_write(
                    ...
                    _buffer: *const u8,
                    _size: usize,
                ) -> crate::platform::FileResult<usize> {
                    Err(())
                }
        ...
        // 测试创建时序表
        let _ = db.create_time_series_table("test_perf", "timestamp", "value", &[], None);
        ...
        // 检查记录大小
        assert_eq!(
            core::mem::size_of::<TimeSeriesRecord>(),
            88,
            "时序记录大小应正确"
        ); // 实际大小因内存对齐为88字节
        ...
        // 检查记录对齐
        assert!(
            core::mem::align_of::<TimeSeriesRecord>() <= 8,
            "时序记录对齐应不超过8字节"
        );
    }
}
...
fn test_config_inheritance() {
    ...
    assert_eq!(
        custom_config.retention_period(),
        base_config.retention_period(),
        "未修改的保留期应继承默认值"
    );
    assert_eq!(
        custom_config.compression, base_config.compression,
        "未修改的压缩类型应继承默认值"
    );
    assert_eq!(
        custom_config.max_partitions, base_config.max_partitions,
        "未修改的最大分区数应继承默认值"
    );
}
