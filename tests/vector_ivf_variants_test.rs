...
fn test_vector_index_ivf_basic() {
    ...
    // 测试创建IVF索引
    println!("测试创建IVF基础索引");
    let ivf_result = db.sql_query("CREATE INDEX ivf_basic_idx ON IVF_VARIANTS_TABLE (vector) USING IVF");
    if ivf_result.is_ok() {
        println!("  成功创建IVF基础索引");
    } else {
    ...
    // 测试向量查询
    println!("测试向量查询");
    let vector_result = db.sql_query("SELECT id FROM IVF_VARIANTS_TABLE WHERE vector <-> [2.0, 4.0, 6.0, 8.0] < 5.0");
...
fn test_vector_index_ivf_pq() {
    ...
    for i in 1..=12 {
        let record = IVFVectorRecord {
            id: i,
            vector: [i as f32 * 0.5, i as f32 * 1.0, i as f32 * 1.5, i as f32 * 2.0],
            group_id: i % 4 + 1 // 分成4组
        };
    ...
    let ivf_pq_result = db.sql_query("CREATE INDEX ivf_pq_idx ON IVF_VARIANTS_TABLE (vector) USING IVF_PQ");
    ...
    // 测试向量查询
    println!("测试向量查询");
    let vector_result = db.sql_query("SELECT id FROM IVF_VARIANTS_TABLE WHERE vector <#> [3.0, 6.0, 9.0, 12.0] > 0.0");
    if vector_result.is_ok() {
...
fn test_vector_index_ivf_variants_combination() {
    ...
    // 首先创建IVF索引
    let ivf_result = db.sql_query("CREATE INDEX ivf_comb_idx ON IVF_VARIANTS_TABLE (vector) USING IVF");
    if ivf_result.is_ok() {
        println!("  成功创建IVF索引");
    } else {
        println!("  创建IVF索引失败");
    ...
    // 测试2: 基本向量查询
    println!("测试2: 基本向量查询");
    let basic_query = db.sql_query("SELECT id FROM IVF_VARIANTS_TABLE WHERE vector <-> [2.0, 4.0, 6.0, 8.0] < 3.0");
    ...
    // L2距离查询
    let l2_result = db.sql_query("SELECT id FROM IVF_VARIANTS_TABLE WHERE vector <-> [3.0, 6.0, 9.0, 12.0] < 5.0");
    if l2_result.is_ok() {
        println!("  L2距离查询成功");
    } else {
        println!("  L2距离查询失败");
    ...
    // IP距离查询
    let ip_result = db.sql_query("SELECT id FROM IVF_VARIANTS_TABLE WHERE vector <#> [3.0, 6.0, 9.0, 12.0] > 100.0");
    if ip_result.is_ok() {
        println!("  IP距离查询成功");
    } else {
        println!("  IP距离查询失败");
    ...
    // Cosine距离查询
    let cosine_result = db.sql_query("SELECT id FROM IVF_VARIANTS_TABLE WHERE vector <=> [3.0, 6.0, 9.0, 12.0] > 0.95");
...
fn test_vector_index_ivf_compatibility() {
    ...
    for i in 1..=9 {
        let record = IVFVectorRecord {
            id: i,
            vector: [i as f32 * 1.1, i as f32 * 2.2, i as f32 * 3.3, i as f32 * 4.4],
            group_id: i / 3 + 1 // 分成3组
        };
    ...
    // 创建IVF索引
    let ivf_result = db.sql_query("CREATE INDEX ivf_compat_idx ON IVF_VARIANTS_TABLE (vector) USING IVF");
    if ivf_result.is_ok() {
        println!("  成功创建IVF索引");
    } else {
        println!("  创建IVF索引失败");
