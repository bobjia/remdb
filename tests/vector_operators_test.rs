...
fn test_vector_operators_basic() {
    ...
    // 测试 Cosine 距离操作符 <=>
    let cosine_result = db
        .sql_query("SELECT id FROM VECTOR_OPERATORS_TABLE WHERE vector3 <=> [1.0, 2.0, 3.0] > 0.5");
...
fn test_vector_operators_with_data_types() {
    ...
    let idx_result = db.sql_query(
        "CREATE INDEX vector_operators_idx ON VECTOR_OPERATORS_TABLE (vector4) USING HNSW",
    );
    if idx_result.is_ok() {
        println!("成功创建向量索引");
    }
...
fn test_vector_operators_different_dimensions() {
    ...
    // 测试2: 4维向量操作符
    println!("测试2: 4维向量操作符");
    let vec4_result = db.sql_query(
        "SELECT id FROM VECTOR_OPERATORS_TABLE WHERE vector4 <-> [1.0, 2.0, 3.0, 4.0] < 1.0",
    );
