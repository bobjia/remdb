    ...
    if is_compressed {
        match compression_scheme {
            ...
            2 => {
                ...
                // 实际实现需要引入 ZSTD 库支持
                unsafe {
                    core::ptr::copy_nonoverlapping(input as *const u8, output.add(4), dimension * size_of::<f32>());
            ...
            _ => {
                // 默认不压缩
                unsafe {
                    core::ptr::copy_nonoverlapping(input as *const u8, output, dimension * size_of::<f32>());
                }
...
mod tests {
    ...
    fn test_vector_compression() {
        ...
        let compressed_size = compress_vector(test_vector.as_ptr(), dimension, compressed.as_mut_ptr());        
        // 测试解压缩
        ...
        for (i, (orig, decomp)) in test_vector.iter().zip(decompressed.iter()).enumerate() {
            assert!((orig - decomp).abs() < 0.001, "Index {}: orig={}, decomp={}", i, orig, decomp);
        }
    }
}
