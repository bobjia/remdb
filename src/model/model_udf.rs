...
#[cfg(feature = "model-runtime")]
use crate::model::cache::{get_or_init_cache, CacheKey};
...
impl ModelUDF {
    ...
    pub fn execute(&self, args: &[TypedValue]) -> core::result::Result<TypedValue, String> {
        ...
        for arg in args {
            #[cfg(feature = "log")]
            debug!(
                "ModelUDF::execute: processing arg with type {:?}",
                arg.value_type
            );
        ...
        #[cfg(feature = "log")]
        debug!(
            "ModelUDF::execute: after processing args, model_inputs len: {}",
            model_inputs.len()
        );
        ...
        #[cfg(feature = "model-runtime")]
        {
            ...
            #[cfg(feature = "log")]
            debug!(
                "ModelUDF::execute: after executing model, output len: {}",
                output.len()
            );
            ...
            #[cfg(feature = "log")]
            debug!(
                "ModelUDF::execute: cache miss, result cached for model {}",
                self.name
            );
        }
        ...
        #[cfg(feature = "log")]
        debug!(
            "ModelUDF::execute: before returning, typed_value type: {:?}",
    ...
    fn execute_via_worker(&self, args: &[TypedValue]) -> core::result::Result<TypedValue, String> {
        ...
        #[cfg(feature = "log")]
        debug!(
            "ModelUDF::execute_via_worker: start for model {}",
            self.name
        );
        ...
        if let Some(_entry) = cache.get(&cache_key) {
            #[cfg(feature = "log")]
            debug!(
                "ModelUDF::execute_via_worker: cache hit for model {}",
                self.name
            );
        ...
        let mut manager_guard =
            get_worker_manager().map_err(|_| "Worker manager unavailable".to_string())?;
        ...
        match response {
            ModelResponse::ExecutionResult { output } => {
                ...
                #[cfg(feature = "log")]
                debug!(
                    "ModelUDF::execute_via_worker: cache miss, result cached for model {}",
                    self.name
                );
...
pub fn execute_model_udf(
    name: &str,
    args: &[TypedValue],
) -> Result<crate::types::TypedValue, crate::sql::QueryExecutionError> {
    use crate::model::model_manager::get_global_model_manager;

    let model_manager =
        get_global_model_manager().map_err(|_| crate::sql::QueryExecutionError::InternalError)?;

    if model_manager.is_using_worker() {
        let model_udf = ModelUDF::new_with_worker(name.to_string());
        model_udf.execute(args).map_err(|e| {
            #[cfg(feature = "log")]
