...
pub fn get_cache_dir() -> std::path::PathBuf {
    let cache_dir = std::env::var("REMDB_MODEL_CACHE").unwrap_or_else(|_| {
...
pub async fn download_model(
    ...
    if let Some(parent) = dest.parent() {
        std::fs::create_dir_all(parent).map_err(|e| DownloadError::IoError(e.to_string()))?;
    }

    let mut file =
        std::fs::File::create(dest).map_err(|e| DownloadError::IoError(e.to_string()))?;
    ...
    while let Some(chunk) = stream.next().await {
        ...
        if let Some(ref callback) = progress_callback {
            let percentage =
                total_bytes.map(|total| (downloaded_bytes as f32 / total as f32) * 100.0);
