...
impl TTLCircularBuffer {
    ...
    pub fn write(&mut self, data: &[u8], ttl_ms: u64) -> bool {
        ...
        // 检查槽位是否可用（使用索引访问，避免借用冲突）
        let state = self.buffer[slot_idx].state.load(Ordering::Acquire);
        ...
        // 开始写入
        if !slot.state.compare_exchange(SLOT_FREE, SLOT_WRITING, Ordering::AcqRel, Ordering::Acquire).is_ok() {
            return false;
        }

        // 存储数据
        slot.data = Vec::from(data);
    ...
    pub fn read(&mut self, buffer: &mut [u8]) -> Option<usize> {
        ...
        loop {
            ...
            // 检查是否过期
            let expire_time = self.buffer[slot_idx].timestamp.load(Ordering::Acquire);
            if expire_time <= now {
                // 过期，释放内存并标记为删除
                let slot = &mut self.buffer[slot_idx];
                slot.data.clear();
            ...
            buffer[..data_len].copy_from_slice(&slot.data[..data_len]);
    ...
    pub fn evict_expired(&mut self, now: u64) -> bool {
        // 从堆顶开始检查过期数据
        while let Some(Reverse(entry)) = self.ttl_heap.peek().cloned() {
            // 检查堆顶元素是否过期
            ...
            // 检查该槽位的实际状态和逻辑索引
            let slot = &mut self.buffer[entry.slot_idx];
    ...
    pub fn evict_shortest_ttl(&mut self, now: u64) -> bool {
        ...
        while let Some(Reverse(entry)) = heap_copy.pop() {
            let slot = &self.buffer[entry.slot_idx];
            let state = slot.state.load(Ordering::Acquire);
    ...
    /// 回退方案：线性扫描找到最短TTL的数据
    fn evict_shortest_ttl_fallback(&mut self, now: u64) -> bool {
