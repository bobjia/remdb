...
pub fn compress_wal_data(data: &[u8], compression_type: WALCompressionType, _level: u8) -> Result<Vec<u8>> {
    match compression_type {
        ...
        WALCompressionType::LZ4 => {
            Err(RemDbError::InvalidConfig("LZ4 compression feature not enabled".to_string()))
        }
        ...
        WALCompressionType::ZSTD => {
            Err(RemDbError::InvalidConfig("ZSTD compression feature not enabled".to_string()))
        }
    }
}
