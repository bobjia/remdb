...
pub unsafe fn init_system_tables(db: &mut crate::RemDb) -> Result<bool> {
    ...
    let system_config_table_exists = db.tables.iter().any(|table_opt| {
        table_opt
            .as_ref()
            .map(|table| table.def.name == SYSTEM_CONFIG_TABLE)
            .unwrap_or(false)
    });
...
unsafe fn insert_default_configs(db: &mut crate::RemDb) -> Result<()> {
    ...
    let default_configs = [
        ...
        (
            ...
            "none",
            "向量压缩方案：none=不压缩, float16=float16, zstd=ZSTD",
        ),
...
unsafe fn insert_default_rbac_data(db: &mut crate::RemDb) -> Result<()> {
    ...
    // 2. 插入 admin 角色的所有权限
    let role_permissions_table_id = db
        .tables
        .iter()
        .position(|table_opt| {
            table_opt
