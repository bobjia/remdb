...
impl IndexBuildThreadPool {
    ...
    fn execute_index_build(task: IndexBuildTask) {
        ...
        // 2. 分配索引内存
        // 根据索引类型分配不同的内存
        let index = match task.sql_index_type {
            SqlIndexType::BTree => {
                // 分配BTree索引内存
                let btree_index = crate::index::BTreeIndex::new();
                crate::index::AnySecondaryIndex::BTree(btree_index)
            }
            SqlIndexType::TTree => {
                // 分配TTree索引内存
                let ttree_index = crate::index::TTreeIndex::new();
                crate::index::AnySecondaryIndex::TTree(ttree_index)
            }
            SqlIndexType::Vector => {
                // 向量索引需要特殊处理
                println!("Error: Vector index building not supported yet");
                return;
            }
        };
        ...
        let total_rows = table.record_count;
        
        for i in 0..table.def.max_records {
            // 检查是否已取消
            if task.canceled.load(core::sync::atomic::Ordering::SeqCst) {
                println!("Index build task {:?} canceled", task.id);
                return;
            }
            
            // 检查记录是否已使用
            unsafe {
                let status_ptr = table.status_array.as_ptr().add(i);
            ...
            let record_ptr = unsafe {
                table.data_start.as_ptr().add(i * table.record_size)
            };
            
            // 4. 为每条记录构建索引项
            // 构建索引键
            let mut key = Vec::new();
            for column_name in &task.column_name {
                // 查找字段索引
                let field_index = table.def.fields.iter()
                    .position(|f| f.name == *column_name)
                    .unwrap_or_else(|| {
                        println!("Error: Column {} not found", column_name);
                        return;
                    });
                
                let field = &table.def.fields[field_index];
                let field_ptr = unsafe {
                    record_ptr.add(field.offset)
                };
                
                // 根据字段类型构建键
                match field.data_type {
                    crate::types::DataType::UInt8 => {
                        let value = unsafe { *field_ptr as u8 };
                        key.extend_from_slice(&value.to_le_bytes());
                    }
                    crate::types::DataType::UInt16 => {
                        let value = unsafe { core::ptr::read_unaligned(field_ptr as *const u16) };
                        key.extend_from_slice(&value.to_le_bytes());
                    }
                    crate::types::DataType::UInt32 => {
                        let value = unsafe { core::ptr::read_unaligned(field_ptr as *const u32) };
                        key.extend_from_slice(&value.to_le_bytes());
                    }
                    crate::types::DataType::UInt64 => {
                        let value = unsafe { core::ptr::read_unaligned(field_ptr as *const u64) };
                        key.extend_from_slice(&value.to_le_bytes());
                    }
                    crate::types::DataType::Int8 => {
                        let value = unsafe { core::ptr::read_unaligned(field_ptr as *const i8) };
                        key.extend_from_slice(&value.to_le_bytes());
                    }
                    crate::types::DataType::Int16 => {
                        let value = unsafe { core::ptr::read_unaligned(field_ptr as *const i16) };
                        key.extend_from_slice(&value.to_le_bytes());
                    }
                    crate::types::DataType::Int32 => {
                        let value = unsafe { core::ptr::read_unaligned(field_ptr as *const i32) };
                        key.extend_from_slice(&value.to_le_bytes());
                    }
                    crate::types::DataType::Int64 => {
                        let value = unsafe { core::ptr::read_unaligned(field_ptr as *const i64) };
                        key.extend_from_slice(&value.to_le_bytes());
                    }
                    crate::types::DataType::Float32 => {
                        let value = unsafe { core::ptr::read_unaligned(field_ptr as *const f32) };
                        key.extend_from_slice(&value.to_le_bytes());
                    }
                    crate::types::DataType::Float64 => {
                        let value = unsafe { core::ptr::read_unaligned(field_ptr as *const f64) };
                        key.extend_from_slice(&value.to_le_bytes());
                    }
                    crate::types::DataType::Bool => {
                        let value = unsafe { *field_ptr != 0 };
                        key.push(value as u8);
                    }
                    crate::types::DataType::VarChar | crate::types::DataType::Char | crate::types::DataType::Text => {
                        // 读取字符串
                        let bytes = unsafe { core::slice::from_raw_parts(field_ptr, field.size) };
                        let end = bytes.iter().position(|b| *b == 0).unwrap_or(bytes.len());
                        key.extend_from_slice(&bytes[..end]);
                        key.push(0); // 字符串结束符
                    }
                    _ => {
                        println!("Error: Unsupported data type for index");
                        return;
                    }
                }
            }
            ...
            // 添加索引项
            unsafe {
                index.insert(key.as_ptr(), key.len(), i as u16);
            }
            
            // 5. 更新索引构建状态
            processed_rows += 1;
            let progress = (processed_rows as f64 / total_rows as f64 * 100.0) as usize;
            
            // 6. 构建完成后，将索引添加到表中
            if processed_rows == total_rows {
                // 获取数据库的可变引用
                let mut db = unsafe { crate::get_global_db() };
                if db.is_some() {
                    let mut db = db.unwrap();
                    // 确保表ID有效
                    if table_id < db.secondary_indices.len() {
                        db.secondary_indices[table_id] = Some(index);
                        println!("Index build task {:?} completed successfully", task.id);
                    }
                }
            }
        }
