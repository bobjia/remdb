...
fn main() {
    ...
    #[cfg(feature = "pubsub")]
    {
        ...
        pubsub
            .register_topic(METRICS_TOPIC, METRICS_TOPIC_ID)
            .expect("Failed to register metrics topic");
        pubsub
            .register_topic(HEALTH_STATUS_TOPIC, HEALTH_STATUS_TOPIC_ID)
            .expect("Failed to register health status topic");
    }
    ...
    println!(
        "- HEALTH_STATUS (ID: {}) - Health status updates",
        HEALTH_STATUS_TOPIC_ID
    );
    ...
    #[cfg(feature = "pubsub")]
    {
        let _wal_thread = thread::spawn(move || {
            ...
            while *running_clone_wal.lock().unwrap() {
                ...
                let wal_data = format!(
                    "WAL_LOG_{}: Operation={}, Table=test_table, ID={}, Data={}",
                    log_id,
                    op_type,
                    log_id,
                ...
                // 发布到单一WAL主题
                match server_clone_wal
                    .lock()
                    .unwrap()
                    .publish(WAL_TOPIC_ID, wal_data.as_bytes())
                {
    ...
    let _tables_thread = thread::spawn(move || {
        ...
        while *running_clone_tables.lock().unwrap() {
            ...
            match server_clone_tables
                .lock()
                .unwrap()
                .publish(TABLES_TOPIC_ID, tables_event.as_bytes())
            {
