...
fn example_bge_small_zh_model() {
    ...
    if !model_path.exists() {
        println!("BGE-Small-ZH-v1.5 model file not found at '{}'", BGE_SMALL_ZH_MODEL_PATH);
    ...
    println!("Loading BGE-Small-ZH-v1.5 model from '{}'...", BGE_SMALL_ZH_MODEL_PATH);
    ...
    match OnnxModel::load(BGE_SMALL_ZH_MODEL_PATH) {
        Ok(model) => {
            ...
            if model.is_bert_style() {
                ...
                match model.execute_int64_batch(&batch_data) {
                    Ok(outputs) => {
                        ...
                        println!("  Output dimensions: {:?}", outputs.iter().map(|o| o.len()).collect::<Vec<_>>());
                        println!("  Batch inference time: {:?}", elapsed);
                    }
                    Err(e) => println!("  Failed to execute batch inference: {}", e),
                }
            } else {
                println!("\nExecuting single inference with {}-dim input...", BGE_SMALL_ZH_DIMENSION);
            ...
            match manager.register_model(
                ...
                vec![("text".to_string(), "STRING".to_string())],
                ("embedding".to_string(), format!("VECTOR({})", BGE_SMALL_ZH_DIMENSION)),
            ) {
                Ok(()) => {
