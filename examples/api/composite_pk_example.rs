    ...
    unsafe {
        ...
        #[cfg(not(feature = "posix"))]
        {
            ...
            impl remdb::platform::Platform for DummyPlatform {
                ...
                fn file_open(&self, _path: &str, _mode: remdb::platform::FileMode) -> remdb::platform::FileResult<remdb::platform::FileHandle> { Err(()) }
                fn file_close(&self, _handle: remdb::platform::FileHandle) -> remdb::platform::FileResult<()> { Err(()) }
                fn file_write(&self, _handle: remdb::platform::FileHandle, _buffer: *const u8, _size: usize) -> remdb::platform::FileResult<usize> { Err(()) }
                fn file_read(&self, _handle: remdb::platform::FileHandle, _buffer: *mut u8, _size: usize) -> remdb::platform::FileResult<usize> { Err(()) }
                fn file_seek(&self, _handle: remdb::platform::FileHandle, _offset: i64, _whence: remdb::platform::SeekWhence) -> remdb::platform::FileResult<u64> { Err(()) }
                fn file_remove(&self, _path: &str) -> remdb::platform::FileResult<()> { Err(()) }
                fn file_size(&self, _path: &str) -> remdb::platform::FileResult<usize> { Err(()) }
...
fn test_api_composite_pk(db: &mut RemDb) -> Result<()> {
    ...
    let table = tables.iter()
        .find(|table_opt| {
            if let Some(table) = table_opt { table.def.name == "metrics_api" } else { false }
        })
...
fn test_sql_composite_pk(db: &mut RemDb) -> Result<()> {
    ...
    let table = tables.iter()
        .find(|table_opt| {
            if let Some(table) = table_opt { table.def.name == "metrics_sql" } else { false }
        })
        .ok_or(remdb::RemDbError::RecordNotFound)?
        .as_ref()
        .ok_or(remdb::RemDbError::RecordNotFound)?;
...
fn test_sql_composite_pk_operations(db: &mut RemDb) -> Result<()> {
    ...
    let result = db.sql_query("SELECT * FROM sensor_readings WHERE sensor_id = 1 AND region_id = 100")?;
    println!("{}", result.to_string());
    
    println!("\n按时间范围查询:");
    let result = db.sql_query("SELECT * FROM sensor_readings WHERE reading_time >= 1609459260000")?;
    println!("{}", result.to_string());
    
    println!("\n聚合查询 - 按sensor_id分组统计平均温度:");
    let result = db.sql_query("SELECT sensor_id, AVG(temperature) AS avg_temp FROM sensor_readings GROUP BY sensor_id")?;
    println!("{}", result.to_string());
    ...
    println!("\n测试复合主键唯一性约束...");
    let duplicate_insert = db.sql_query("INSERT INTO sensor_readings VALUES (1, 100, 1609459200000, 30.0, 50.0)");
    ...
    println!("\n测试不同复合主键组合的插入...");
    let new_insert = db.sql_query("INSERT INTO sensor_readings VALUES (1, 100, 1609459380000, 24.5, 63.0)");
    match new_insert {
    ...
    println!("\n最终数据:");
    let result = db.sql_query("SELECT * FROM sensor_readings ORDER BY sensor_id, region_id, reading_time")?;
    println!("{}", result.to_string());
