...
static USERS: std::sync::LazyLock<remdb::types::TableDef> =
...
// 定义订单表结构
static ORDERS: std::sync::LazyLock<remdb::types::TableDef> =
    std::sync::LazyLock::new(|| remdb::types::TableDef {
        id: 1,
...
// 定义产品表结构
static PRODUCTS: std::sync::LazyLock<remdb::types::TableDef> =
    std::sync::LazyLock::new(|| remdb::types::TableDef {
        id: 2,
...
fn main() {
    unsafe {
        ...
        println!("\n4. 活跃用户:");
        let active_users = db
            .sql_query("SELECT id, name, email, age FROM users WHERE active = true")
            .unwrap();
        ...
        let update_result = db
            .sql_query("UPDATE products SET stock = stock - 1 WHERE name = 'Laptop'")
            .unwrap();
        ...
        println!("\n2. 更新用户状态:");
        let update_result = db
            .sql_query("UPDATE users SET active = false WHERE age > 30")
            .unwrap();
        println!("更新结果: {}", update_result.to_string());
        ...
        println!("\n3. 更新订单状态:");
        let update_result = db
            .sql_query("UPDATE orders SET status = 'completed' WHERE status = 'pending'")
            .unwrap();
        println!("更新结果: {}", update_result.to_string());

        // 验证更新结果
        println!("\n4. 验证更新结果:");
        let updated_products = db
            .sql_query("SELECT name, stock FROM products WHERE name = 'Laptop'")
            .unwrap();
        println!("Laptop库存: {}", updated_products.to_string());

        let inactive_users = db
            .sql_query("SELECT id, name, age, active FROM users WHERE active = false")
            .unwrap();
        println!("非活跃用户: {}", inactive_users.to_string());

        let completed_orders = db
            .sql_query("SELECT id, product, status FROM orders WHERE status = 'completed'")
            .unwrap();
        ...
        let delete_result = db
            .sql_query("DELETE FROM orders WHERE amount < 500")
            .unwrap();
        ...
        println!("\n3. 删除库存为0的产品（示例）:");
        let delete_result = db
            .sql_query("DELETE FROM products WHERE stock <= 0")
            .unwrap();
        println!("删除结果: {}", delete_result.to_string());

        // 验证删除结果
        println!("\n验证删除结果:");
        let remaining_users = db
            .sql_query("SELECT COUNT(*) as user_count FROM users")
            .unwrap();
        let remaining_orders = db
            .sql_query("SELECT COUNT(*) as order_count FROM orders")
            .unwrap();
        let remaining_products = db
