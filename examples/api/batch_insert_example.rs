...
fn main() -> Result<()> {
    ...
    // 1. 创建测试表
    println!("1. 创建测试表");
    db.sql_query("CREATE TABLE products (id INT32 PRIMARY KEY, name TEXT, price REAL, stock INT32)")?;
    ...
    for i in 1..=100 {
        let sql = format!("INSERT INTO products VALUES ({}, 'Product_{}', {}, {})", i, i, i as f64 * 10.0, i * 10);
        db.sql_query(&sql)?;
    }
    ...
    for i in 1..=100 {
        ...
        values_str.push_str(&format!("({}, 'Product_{}', {}, {})", i, i, i as f64 * 10.0, i * 10));
    }
    ...
    println!("   batch_insert_record 插入 {} 条记录耗时: {:?}", affected_rows, api_batch_duration);
    ...
    println!("   批量插入 {} 条记录耗时: {:?}", affected_rows, large_batch_duration);
