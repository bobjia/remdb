...
fn main() {
    unsafe {
        ...
        { // 开始一个作用域，限制table_mut的生命周期
            ...
            println!("涉及 {} 个传感器，每个传感器 {} 条历史数据", sensors.len(), 100);
            ...
            for i in 0..5.min(latest_count) {
                ...
                core::ptr::copy_nonoverlapping(record.as_ptr().add(4), sensor_id_str.as_mut_ptr(), 32);
                let sensor_id = String::from_utf8_lossy(&sensor_id_str).trim_end_matches(char::from(0)).to_string();
                
                let mut sensor_type_str = [0u8; 32];
                core::ptr::copy_nonoverlapping(record.as_ptr().add(36), sensor_type_str.as_mut_ptr(), 32);
                let sensor_type = String::from_utf8_lossy(&sensor_type_str).trim_end_matches(char::from(0)).to_string();
                
                let value = core::ptr::read(record.as_ptr().add(68) as *const f64);
                let timestamp = core::ptr::read(record.as_ptr().add(76) as *const u64);
                
                println!("  记录 {}: ID={}, 传感器={}, 类型={}, 数值={:.2}, 时间戳={}", 
                         i+1, id, sensor_id, sensor_type, value, timestamp);
            }
            ...
            let temp_count = table_mut.get_records_in_time_window(
                4, // timestamp字段索引
                start_time,
                end_time,
                &mut temp_buffer,
            ...
            for (i, (window_start, _sum, avg, min, max, count)) in window_aggregates.iter().enumerate() {
        ...
        { // 开始一个作用域，获取表引用
            ...
            for (sensor_id, sensor_type, location) in sensors.iter() {
                // 统计该传感器的记录数
                let count = table_mut.aggregate_count(
                    4, // timestamp字段索引
                    base_time - 10 * 60 * 1000, // 10分钟前
                    base_time // 当前时间
                ).unwrap();
                
                println!("  {} ({} - {}): {} 条记录", sensor_id, sensor_type, location, count);
            }
        }
        ...
        println!("  20分钟前的数据是否过期: {}", lifecycle_manager.is_expired(expired_time / 1000));
        println!("  5分钟前的数据是否过期: {}", lifecycle_manager.is_expired(recent_time / 1000));
        ...
        println!("  压缩率: {:.2}%", (1.0 - (compressed.len() as f64 / (int_values.len() * 8) as f64)) * 100.0);
