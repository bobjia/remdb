...
use remdb::{
    config::{DbConfig, HAConfig, MemoryAllocator, WALConfig},
    RemDb,
};
use std::env;
...
fn main() {
    ...
    if args.len() > 1 {
        match args[1].as_str() {
            ...
            _ => {
                ...
                println!(
                    "Usage: {} [master|slave] [replication_mode] [master_ip] [master_port]",
                    args[0]
                );
        ...
        if role == HARole::Slave {
            ...
            if args.len() < required_args {
                ...
                println!(
                    "Usage: {} slave [sync|async] <master_ip> <master_port>",
                    args[0]
                );
                return;
            }
    ...
    remdb::memory::allocator::init_global_allocator(ptr, mem_buffer.len())
    ...
    // 注册所有预定义主题
    pubsub
        .register_topic(WAL_TOPIC, WAL_TOPIC_ID)
        .expect("Failed to register WAL topic");
    pubsub
        .register_topic(TABLES_TOPIC, TABLES_TOPIC_ID)
        .expect("Failed to register tables topic");
    ...
    println!(
        "- TABLES (ID: {}) - Table creation/deletion events",
        TABLES_TOPIC_ID
    );
    ...
    if role == HARole::Master {
        ...
        let _sql_thread = thread::spawn(move || {
            ...
            while *running_clone_sql.lock().unwrap() {
                ...
                match db.lock().unwrap().sql_query(&insert_sql) {
                    Ok(result) => {
                        ...
                        match server_clone
                            .lock()
                            .unwrap()
                            .publish(table_topic_id, table_data.as_bytes())
                        {
                ...
                if counter % 10 == 0 {
                    let update_sql =
                        format!("UPDATE users SET age = age + 1 WHERE id = {}", counter - 5);
                    match db.lock().unwrap().sql_query(&update_sql) {
                        Ok(result) => {
                            println!("Updated record: {}", result.to_string());
                            // 发布表内容变更
                            let table_data =
                                format!("UPDATE: Table=users, Record=User{}", counter - 5);
                            match server_clone
                                .lock()
                                .unwrap()
                                .publish(table_topic_id, table_data.as_bytes())
                            {
                                Ok(_) => {
                                    println!("Published {}: {}", users_table_topic, table_data)
                                }
                                Err(e) => {
                                    println!("Failed to publish {}: {:?}", users_table_topic, e)
                                }
                            }
                        }
                ...
                if counter % 15 == 0 {
                    ...
                    match db.lock().unwrap().sql_query(&delete_sql) {
                        Ok(result) => {
                            ...
                            // 发布表内容变更
                            let table_data =
                                format!("DELETE: Table=users, Record=User{}", counter - 10);
                            match server_clone
                                .lock()
                                .unwrap()
                                .publish(table_topic_id, table_data.as_bytes())
    ...
    let _metrics_thread = thread::spawn(move || {
        ...
        while *running_clone_metrics.lock().unwrap() {
            ...
            match server_clone_metrics
                .lock()
                .unwrap()
                .publish(METRICS_TOPIC_ID, metrics_data.as_bytes())
            {
    ...
    let _health_thread = thread::spawn(move || {
        ...
        while *running_clone_health.lock().unwrap() {
            ...
            match server_clone_health
                .lock()
                .unwrap()
                .publish(HEALTH_STATUS_TOPIC_ID, health_data.as_bytes())
            {
