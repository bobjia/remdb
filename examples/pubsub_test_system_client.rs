...
fn main() {
    ...
    // 订阅所有WAL主题
    pubsub.subscribe(1, wal_callback.clone()).expect("Failed to subscribe to WAL_INSERT topic");
    pubsub.subscribe(2, wal_callback.clone()).expect("Failed to subscribe to WAL_UPDATE topic");
    pubsub.subscribe(3, wal_callback.clone()).expect("Failed to subscribe to WAL_DELETE topic");
    pubsub.subscribe(4, wal_callback.clone()).expect("Failed to subscribe to WAL_TIMESERIES_INSERT topic");
    pubsub.subscribe(5, wal_callback.clone()).expect("Failed to subscribe to WAL_COMMIT topic");
    pubsub.subscribe(6, wal_callback.clone()).expect("Failed to subscribe to WAL_ABORT topic");
    pubsub.subscribe(7, wal_callback.clone()).expect("Failed to subscribe to WAL_CHECKPOINT topic");
    pubsub.subscribe(8, wal_callback.clone()).expect("Failed to subscribe to WAL_ALL topic");
    ...
    // 订阅表内容主题
    pubsub.subscribe(12, table_callback).expect("Failed to subscribe to table.test_table topic");
    ...
    // 订阅表创建/删除事件主题
    pubsub.subscribe(9, tables_callback).expect("Failed to subscribe to TABLES topic");
    ...
    // 订阅指标主题
    pubsub.subscribe(10, metrics_callback).expect("Failed to subscribe to METRICS topic");
    ...
    // 订阅健康状态主题
    pubsub.subscribe(11, health_callback).expect("Failed to subscribe to HEALTH_STATUS topic");
